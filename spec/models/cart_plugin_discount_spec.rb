...
RSpec.describe CartPluginDiscount, type: :model do
  ...
  describe "discount if above condtion" do
    it "滿額折X元" do
      ...
      expect(cart.get_total.special).to be 1100 # 1200 - 100
      cart.add_item p1.id # + 100
      expect(cart.get_total.special).to be 1200 # 再測一次，確認沒有重複扣
      cart.update_quantity(p1.id, 1) # 更新未達門檻
      expect(cart.get_total.special).to be 900
    end
    ...
    it "滿額X% off" do
      ...
      expect(cart.get_total.special).to be 2400 # 3000 - (3000 * 0.2) = 2400
